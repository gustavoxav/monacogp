import type { Car, Particle, Player } from '@/types/game'
...
export class GameRenderer {
  ...
  drawCar(
    ...
    // Windshield
    this.ctx.fillStyle = isPlayer ? 'rgba(0, 245, 255, 0.3)' : 'rgba(0, 0, 0, 0.4)'
    this.ctx.fillRect(x - width / 2 + 3, y - height / 2 + 5, width - 6, height * 0.3)
  ...
  drawHUD(distance: number, speed: number, highScore: number, canvasWidth: number) {
    this.ctx.save()
    ...
    this.ctx.fillText(`VELOCIDADE: ${(speed * 10).toFixed(0)}`, canvasWidth - 20, 80)

    if (highScore > 0) {
      this.ctx.fillStyle = '#ff006e'
      this.ctx.fillText(`RECORDE: ${highScore}m`, canvasWidth - 20, 105)
    }
    this.ctx.restore()
  }
