...
export class GameRenderer {
  ...
  drawCar(x: number, y: number, width: number, height: number, color: string, isPlayer = false) {
    this.ctx.save()
    ...
    // Windshield
    this.ctx.fillStyle = isPlayer ? "rgba(255, 255, 255, 0.3)" : "rgba(0, 0, 0, 0.4)"
  ...
  drawHUD(distance: number, speed: number, highScore: number, carsPassed: number, canvasWidth: number) {
    this.ctx.save()
